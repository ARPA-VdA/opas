...
$(document).ready(function() {
    ...
    $( "#btn-add-match" ).click(function() {
        ...
        clearfields();
        $('#match-form').attr('action', 'man_allocation_match_new');
    });
    ...
    $( ".row" ).on( "click", ".view-tool", function(e) {
        ...
        var id = $(this).parent().parent().data("steqid");
        console.log('id:'+id);
        loadReport(id, 'show');
        e.preventDefault();
        return false;
    ...
    $( ".row" ).on( "click", ".set-tool", function(e) {
        ...
        $('#st-eq-id').val(id);
        loadReport(id, 'edit');
        $('#match-form').attr('action', 'man_allocation_match_edit');
        e.preventDefault();
        return false;
    });
    $( ".row" ).on( "click", ".delete-tool", function(e) {
        ...
        swal({
            ...
            var jqxhr = $.ajax({
                url: 'man_allocation_match_delete',
                type: "post",
                dataType: "json",
                data: { id: id },
            })
    ...
    $( "#refresh-list" ).click(function(e) {
        e.preventDefault();
        $.redirectPost( "man_allocation_match", {
            startDate: startDate,
            endDate : endDate
        } );
        return false;
    ...
    $( "#download-csv" ).click(function(e) {
        e.preventDefault();
        $.redirectPost( "man_allocation_match_csv", {
            startDate: startDate,
            endDate : endDate
        } );
        return false;
...
function loadReport(id, dest) {
    ...
    .done(function(result) {
        console.dir( result );
        if (dest == 'show')
            viewTank(result.match);
        if (dest == 'edit')
            editTank(result.match);
    })
    .fail(function(xhr, err) {
        return null;
    });
...
function viewTank(report) {
    ...
    console.dir(report);
    $("#view-equipment").text(report.description+" ("+report.arpa_id+") "+report.ch_values);
    $("#view-workstation").text(report.station);
